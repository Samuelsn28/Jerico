<?php
	...
	$stmt_pega_contas = mysqli_prepare($conexao, "SELECT * FROM $tabelaConta WHERE id_usuario=?");
	$stmt_cria_conta = mysqli_prepare($conexao, "INSERT INTO $tabelaConta (id, id_usuario, plataforma, url, email_conta, login, senha, observacoes) VALUES (0, ?, ?, ?, ?, ?, ?, ?)");
	...
	function pega_usuario($email) {
		...
		return mysqli_stmt_get_result($stmt_pega_usuario);
	}
